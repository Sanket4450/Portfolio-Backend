export default {
    MESSAGES: {
        SUCCESS: {
            MESSAGE_SENT: 'Message sent successfully',
            SESSION_LOGIN_OTP: 'Admin Session Login OTP',
            SECRET_VERIFIED: 'Admin secret verified successfully',
            OTP_SENT: 'Session Login OTP sent successfully',
            OTP_VERIFIED: 'Session Login OTP verified successfully',
            MESSAGES_FETCHED: 'Messages fetched successfully',
            MESSAGE_FETCHED: 'Message fetched successfully',
            MARKED_ALL_AS_READ: 'All messages marked as read',
            MESSAGE_DELETED: 'Message deleted successfully',
            REPLY_MESSAGE: 'Reply Message',
            REPLIES_FETCHED: 'Replies fetched successfully',
            REPLY_MESSAGE_SENT: 'Reply message sent successfully',
            ROOT_DATA_FETCHED: 'Root data fetched successfully',
            TEST_SUCCESS: 'Test successfull',
        },
        ERROR: {
            SOMETHING_WENT_WRONG: 'Something went wrong, please try again',
            MAX_MESSAGE_LIMIT: 'You have reached the maximum message limit',
            INVALID_SECRET: 'Invalid admin secret',
            INVALID_OTP: 'Invalid OTP',
            OTP_EXPIRED: 'OTP has expired! try to generate new OTP',
            INVALID_TOKEN: 'Invalid token or signature',
            TOKEN_EXPIRED: 'Token has expired',
            MESSAGE_NOT_FOUND: 'Message not found for this messageId',
            EMAIL_NOT_DELIVERABLE: 'Email address is not deliverable',
        },
    },
    COLLECTIONS: {
        MESSAGE: 'Message',
        SESSION: 'Session',
        REPLY: 'Reply',
    },
}
